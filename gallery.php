...
$stm = $dbh->prepare('SELECT '. $config['release'] .'.expname, '. $config['release'] .'.ccd, qa.comment, qa.timestamp, users.username FROM qa JOIN '. $config['release'] .' ON ('. $config['release'] .'.rowid=qa.fileid) JOIN users ON (qa.userid = users.rowid) WHERE qa.rowid IN (SELECT MIN(qa.rowid) FROM qa WHERE problem=-1 GROUP BY qa.fileid)');
$stm->execute();
