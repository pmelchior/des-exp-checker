...
$stm = $dbh->prepare('SELECT files.expname, files.ccd, qa.rowid as qa_id, qa.detail, qa.timestamp, users.username FROM qa JOIN files ON (files.rowid=qa.fileid) JOIN users ON (qa.userid = users.rowid) WHERE qa.rowid IN (SELECT MIN(qa.rowid) FROM qa WHERE problem=-1 GROUP BY qa.fileid)');
$stm->execute();
